...
import NovoTrimestreTemplate, { ListaDeAulas, NovoTrimestreBaseInputs } from "../../ui/NovoTrimestreTemplate";
...
const cadastrarNovaLicaoGlobal = httpsCallable(functions, "cadastrarNovaLicaoGlobal");
...
const DragContainer = ({ children }: { children: ReactNode }) => {
    ...
    useEffect(() => {
        ...
        const diff = $dragContainerRef.current.scrollWidth - $dragRef.current.clientWidth;
...
const IgrejasDestinoQtd = React.memo(({ control, totalIgrejas }: { control: Control<any>; totalIgrejas: number }) => {
...
const IgrejasDestino = ({
    ...
    const igrejasMemo = useMemo(() => {
        if (pesquisa) return igrejas.filter((v) => v.nome.toLowerCase().includes(pesquisa));
    ...
    return (
        <div className="matriculas">
            ...
            <div className="matriculas-disponiveis">
                <IgrejasDestinoQtd control={control} totalIgrejas={igrejas?.length} />
                        ...
                        <li key={v.id}>
                            <div className="novo-trimestre-global__check">
                                <input
                                    ...
                                    {...register(`igrejas`)}
                                />
                                <label htmlFor={"igreja-enviar" + v.id}>{v.nome}</label>
                            </div>
                        </li>
                    ))}
                </ul>
...
const Rotulos = ({
    ...
    useEffect(() => {
        ...
        const getUltimaLicao = async () => {
            ...
            const obj: Partial<FormNovoTrimestreGlobal> = {
                data_inicio: dataInicio.toISOString().split("T")[0],
                numero_trimestre: lGlobal.numero_trimestre + 1 > 4 ? 1 : lGlobal.numero_trimestre + 1,
                igrejas: igrejas.map((v) => v.id),
                numero_aulas: 13,
                rotuloId: rotulo || rotuloDefault.id,
            };
    ...
    return (
        <div className="novo-trimestre__input">
            ...
            <Controller
                ...
                render={({ field }) => (
                    <Dropdown
                        current={rotulos.find((v) => v.id === field.value)?.nome || ""}
...
const LicaoGlobalModal = ({
    ...
    return (
        <motion.div className="licao-modal__overlay" onClick={() => window.history.back()}>
            <motion.div
                    ...
                    <div className="licao-modal__header-config">
                        {!!licao.pdf && (
                            <motion.div
                                ...
                                onTap={() => navigate("/ler-pdf", { state: licao.pdf })}
                            >
                                <FontAwesomeIcon icon={faFilePdf} />
                            </motion.div>
                        )}
                        <motion.div className="licao-modal__header--config" onClick={() => onEditLicao(licao.id)}>
...
const NovoTrimestreLicoesGlobais = ({
    ...
    const onSubmit = async (v: FormNovoTrimestreGlobal) => {
        ...
        try {
            if (dadosFinais?.img && dadosFinais.img.length) {
                const imagemRef = await reduzirImagem(dadosFinais.img[0], 800, 800);
    ...
    useEffect(() => {
        ...
        const obj: Partial<FormNovoTrimestreGlobal> = {
            data_inicio: licaoRef.data_inicio.toDate().toISOString().split("T")[0],
        ...
        <>
            ...
            >
                <FormProvider {...methods}>
                    <form onSubmit={handleSubmit(onSubmit)} className="novo-trimestre__form novo-trimestre-global">
                        ...
                        <div className="novo-trimestre__input novo-trimestre__input--file">
                            ...
                            <input type="file" id="pdf_revista" accept=".pdf" {...register("pdf")} />
                        </div>
                        ...
                        <NovoTrimestreBaseInputs nameTrimestre="numero_trimestre" />

                        <ListaDeAulas control={control} nameAulas="numero_aulas" nameData="data_inicio" />

                        <IgrejasDestino control={control} igrejas={igrejas} register={register} />

                        <div className={`novo-trimestre__actions ${licaoRef ? "novo-trimestre__actions--edit" : ""}`}>
                            {!!licaoRef && (
                                <button
                                    type="button"
                                    ...
                                    onClick={() => {
                                        setMensagem({
                                            ...
                                            message: (
                                                <>
                                                    <span>
                                                        Você está prestes a remover a lição{" "}
                                                        <strong>{licaoRef?.titulo}</strong> de todas as igrejas
                                                        vinculadas.
                                                    </span>
                                                    <span>
                                                        Apenas as lições que ainda não possuem nenhuma chamada
                                                        registrada serão excluídas. As demais continuarão normalmente.
                                                    </span>
                                            ...
                                            onConfirm: () => apagarLicao(licaoRef.id),
                                            onCancel: () => {
                                                window.history.back();
                                            },
                                            cancelText: "Cancelar",
...
function LicoesGlobais() {
    ...
    const [editLicao, setEditLicao] = useState<LicaoGlobalInterface | null>(null);
    const [showLicao, setShowLicao] = useState<LicaoGlobalInterface | null>(null);
    const [currentRotulo, setCurrentRotulo] = useState<RotulosClassesInterface | null>(null);
        ...
        if (pesquisa)
            return licoes.filter(
                (v) =>
                    ...
                    `${v.numero_trimestre} trimestre de ${v.data_inicio.toDate().getFullYear()}`.includes(pesquisa),
            );
        return licoes;
    }, [pesquisa, licoes]);
    ...
    useEffect(() => {
        ...
        const getLicoesGlobais = async () => {
            ...
            const rGlobais = rGlobaisDocs.docs
                ...
                .sort((a, b) => b.data_inicio.toDate().getTime() - a.data_inicio.toDate().getTime());
    ...
    useEffect(() => {
        const getRotulos = async () => {
            ...
            const q = query(rotulosCollection, where("ministerioId", "==", user!.ministerioId));
        ...
        <>
                        ...
                        <div className="licoes-globais__lista">
                            ...
                            {licoes.length > 0 ? (
                                <div className="licoes-globais__licoes">
                                    ...
                                    {itensPaginados.totalPaginas > 1 && (
                                        <div className="licoes-grid__paginacao">
                                            ...
                                            <span>
                                                {paginaAtual} de {itensPaginados.totalPaginas}
                                            </span>
                                            <motion.button
                                                onTap={() => setPaginaAtual((p) => p + 1)}
                                                disabled={paginaAtual >= itensPaginados.totalPaginas}
                                            >
                                ...
                                <div className="licoes-globais__licoes-vazio">
                                    <p>Nenhuma lição foi cadastrada para o rótulo {currentRotulo?.nome}</p>
                                    <div className="licoes-grid__header--novo-trimestre">
                                        <motion.button
                                            onTap={() => {
                                                setCriarLicao(true);
                        ...
                        <div className="licoes-globais__sem-rotulos">
                            <div className="licoes-globais__sem-rotulos--texto">
                                <p>
                                    ...
                                    Não existem rótulos de classes cadastrados nesse ministério.
                                </p>
                                <p>Clique no link abaixo para realizar os cadastros</p>
                            </div>

                            <Link to={"/rotulos-classes"} className="licoes-globais__sem-rotulos--link" />
                        </div>
                    )}
                </div>
            </div>
            ...
            <AnimatePresence>
                ...
                {showLicao && !(criarLicao || editLicao) && (
                    <LicaoGlobalModal licao={showLicao} key={"licao-global-modal"} onEditLicao={onEditLicao} />
                )}
            </AnimatePresence>
        </>
    );
