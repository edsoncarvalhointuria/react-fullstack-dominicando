...
import { ListaDeAulas, NovoTrimestreBaseInputs } from "./NovoTrimestreTemplate";
import getTrimestre from "../../utils/getTrimestre";
import getDataInicio from "../../utils/getDataInicio";
...
const ItemAlunoMatriculado = React.memo(
    ({
        ...
        return (
            <motion.li layoutId={alunoId} className={isForaDaFaixa ? "matriculas--fora-da-faixa" : ""}>
                <motion.div className="matriculas-item__nome" onTap={() => remove(index)}>
                    <span>{alunoNome}</span> <span className="matriculas--lista-idade">({alunoIdade} anos)</span>
                </motion.div>

                <div className="matriculas-item__licao">
                    <label htmlFor={"matriculados-revista" + alunoId}>Revista?</label>
                    <input
                        type="checkbox"
                        id={"matriculados-revista" + alunoId}
                        {...register(`alunosSelecionados.${index}.possui_revista`)}
                    />
                </div>
            </motion.li>
        );
            ...
            {listaAlunos.map((v) => (
                ...
                >
                    <motion.p layoutId={v.id}>
                        {v.nome} <span className="matriculas--lista-idade">({v.idade}) anos</span>
                    </motion.p>
                </motion.li>
            ))}
        </ul>
...
function NovoTrimestreModal({
    ...
    const [alunosMap, setAlunosMap] = useState<Map<string, AlunoInterface & { nome: string; idade: number }>>(
        new Map(),
    );
    ...
    const importarAlunos = () => {
        ...
        const filtro = trimestreAnterior.current.filter((v) => !ids.includes(v.alunoId));
    ...
    const deletarRevista = () => {
        setMensagem({
            ...
            message: (
                <>
                    <span>
                        Tem certeza que deseja deletar a lição: <strong>{licaoReference?.titulo}</strong>?
                    </span>
                    <span>
                        Isso irá apagar <strong>TODOS</strong> os dados associados a ela.
                    </span>
                </>
            ),
            onClose: () => {
    ...
    const save = async () => {
        ...
        try {
            ...
            if (dadosFinaisRef.current.img && dadosFinaisRef.current.img.length) {
                const uploadImagem = async () => {
                    const arquivoOriginal = dadosFinaisRef.current.img[0];
                    const arquivo = await reduzirImagem(arquivoOriginal, 800, 800);
                    const storage = getStorage();
                    const caminho = `capas-licoes/${Date.now()}-${arquivo.name}`;
                ...
                if (licaoReference) {
                    if (licaoReference.img !== dadosFinaisRef.current.img) urlImg = await uploadImagem();
    ...
    const onSubmit = async (dados: NovaLicaoForm) => {
        ...
        if (licao) {
            ...
            if (dataUsuario >= dataInicio && dataUsuario < dataFim) {
                return setMensagem({
                    ...
                    message: (
                        <span>
                            A lição anterior <strong>{licao!.titulo}</strong> está com data de finalização prevista para
                            o dia: <strong>{licao!.data_fim.toDate().toLocaleDateString("pt-BR")}</strong>
                            .<br />
                    ...
                    confirmText: "Desativar a lição anterior e cadastrar essa como ativa.",
                    icon: <FontAwesomeIcon icon={faTriangleExclamation} />,
                });
            }
        }
        if (licaoReference) {
            return setMensagem({
                ...
                message: (
                    <span>
                        Você deseja salvar as alterações da lição <strong>{licaoReference!.titulo}</strong>?
                    </span>
                ),
                onClose: () => setMensagem(null),
                onConfirm: () => save(),
    ...
    const listaAlunosMemo = useMemo(() => {
        ...
        const lista = Array.from(alunosMap?.values() || []).filter(
            (v) =>
                !as.includes(v.id) &&
                (v.nome.toLowerCase().includes(pesquisa) || v.idade.toString().includes(pesquisa)),
        );
    ...
    const alunosForaDaIdadeMemo = useMemo(() => {
        ...
        if (classe?.idade_minima || classe?.idade_maxima) {
            fields.forEach((v) => {
                ...
                if (aluno && classe.idade_minima && aluno.idade < classe.idade_minima)
                    alunosForaMap.set(v.alunoId, "abaixo");
                if (aluno && classe.idade_maxima && aluno.idade > classe.idade_maxima)
    ...
    const listaAlunosSelecionadosMemo = useMemo(() => {
        return fields
            .filter((v) => {
                ...
                return aluno?.nome.toLowerCase().includes(pesquisa) || aluno?.idade.toString().includes(pesquisa);
            })
            .sort((a, b) => {
                if (alunosForaDaIdadeMemo.get(a.alunoId)) return -1;
                if (alunosForaDaIdadeMemo.get(b.alunoId)) return 1;

                return alunosMap.get(a.alunoId)!.idade - alunosMap.get(b.alunoId)!.idade;
            });
    }, [alunosMap, pesquisa, fields, alunosForaDaIdadeMemo]);

    const fieldIndex = useCallback((field: any) => fields.findIndex((f) => f.id === field.id), [fields]);
    ...
    useEffect(() => {
        ...
        setMensagem({
            ...
            message: (
                <>
                    <span>
                        Prezado, a classe tem a idade mínima de <strong>{classe?.idade_minima || 0} anos</strong> e
                        idade máxima de <strong>{classe?.idade_maxima} anos</strong>.
                    </span>
                    {abaixo.length > 0 && (
                        <>
                            <br />
                            <span>Os alunos abaixo estão abaixo da idade mínima:</span>
                            <span>
                    ...
                    {acima.length > 0 && (
                        <>
                            <br />
                            <span>Os alunos abaixo estão acima da idade máxima:</span>
                            <span>
    ...
    useEffect(() => {
        const getLicao = async () => {
            if (!licaoReference) {
                ...
                if (licoesSnap.empty) {
                    ...
                    setValue("data_inicio", getDataInicio(trimestre));
                    return null;
                }
                ...
                setValue(
                    "trimestre",
                    (licoes?.numero_trimestre || 0) + 1 === 5 ? 1 : (licoes?.numero_trimestre || 0) + 1,
                );
        ...
        const getTrimestreAnterior = async (licaoId: string, isPrimeiroAcesso?: boolean) => {
            ...
            if (isPrimeiroAcesso) {
                ...
                const q = query(ultimaLicaoCll, ...condicoes);

                const ultimaLicaoDocs = await getDocs(q);
                if (!ultimaLicaoDocs.empty) licaoIdFinal = ultimaLicaoDocs.docs[0].id;
            }

            try {
                const matriculasCollection = doc(db, "cache_matriculas", `${igrejaId}_${licaoIdFinal}`);
        ...
        Promise.all([getLicao(), getAlunos()])
            .then(([licao, aluno]) => {
                ...
                const alunosOrdenados = aluno.sort((a, b) => {
                    const aEstaNaFaixa =
                        a.idade >= (typeof classe?.idade_minima === "number" ? classe!.idade_minima : 0);
                    const bEstaNaFaixa =
                        b.idade >= (typeof classe?.idade_minima === "number" ? classe!.idade_minima : 0);
                ...
                if (licaoReference) {
                    if (licaoReference.primeiroAcesso) {
                        const closeModal = () => {
                            setMensagem(null);
                            document
                                .getElementById("importar-alunos-button")
                        ...
                        setMensagem({
                            message: (
                                <>
                                    <span>
                                        Esta lição foi <strong>cadastrada automaticamente</strong> para você.
                                    </span>
                                    <br />
                                    <span>
                                        <i>
                                        ...
                                        Próximo passo: <strong>matricule os alunos deste trimestre</strong>.
                                    </span>
                                    <br />
                                    <span>
                                        <i>
                                        ...
                                        Dica: use o botão <strong>"Importar alunos do trimestre anterior"</strong> para
                    ...
                    getTrimestreAnterior(licaoReference.id, licaoReference.primeiroAcesso)
                        .then((matricula) => {
                            const result = matricula?.map(
                                (v) =>
                                    ({
                                        ...
                                        nome: v.alunoNome,
                                        idade: getIdade(alunoMap.get(v.alunoId)!.data_nascimento),
                                    }) as any,
                            );
                            trimestreAnterior.current = result;
                            const obj: Partial<NovaLicaoForm> = {
                                alunosSelecionados: licaoReference.primeiroAcesso ? [] : result,
                                data_inicio: licaoReference.data_inicio.toDate().toISOString().split("T")[0],
    ...
    return (
        <>
            <div className="novo-trimestre-close" onClick={() => (!isEnviando ? onClose() : null)}>
                <motion.div
                    ...
                    {isLoading ? (
                        ...
                        <>
                            ...
                            <div className="novo-trimestre__header">
                                    ...
                                    <div className="novo-trimestre__header--infos">
                                        {isSuperAdmin.current && (
                                            <p>
                                                ...
                                                <strong>Igreja:</strong> {classe?.igrejaNome}
                                            </p>
                                        )}
                                        <p>
                                        ...
                                        {(classe?.idade_minima || classe?.idade_maxima) && (
                                            <p>
                                                ...
                                                {typeof classe?.idade_minima === "number"
                                                    ...
                                                    : "N/A"}{" "}
                                                -{" "}
                                                {typeof classe?.idade_maxima === "number"
                                                    ? classe?.idade_maxima
                                                    : "N/A"}{" "}
                                                anos
                                            </p>
                                ...
                                <div className="novo-trimestre__header--close" onClick={() => onClose()}>
                                ...
                                {licaoReference ? (
                                    <div className="novo-trimestre__header--aviso">
                                        ...
                                        <span>Atenção: você está editando a lição</span>
                                    </div>
                                ) : (
                                    licao && (
                                        <div className="novo-trimestre__header--aviso">
                                            <FontAwesomeIcon icon={faTriangleExclamation} />
                                            <span>Já existe uma lição ativa para esta classe.</span>
                                        </div>
                                    )
                                )}
                            </div>
                                    ...
                                    <form onSubmit={handleSubmit(onSubmit)} className="novo-trimestre__form">
                                        ...
                                        <div className="matriculas">
                                            <div className="matriculas-alunos__search">
                                                <SearchInput texto="aluno (nome, idade)" onSearch={setPesquisa} />
                                            </div>
                                            <div className="matriculas-disponiveis">
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => setShowCadastroAluno(true)}
                                                >
                                                    <FontAwesomeIcon icon={faUserPlus} />
                                                    Cadastrar novo aluno
                                                </button>
                                                <AlunosDisponiveis append={append} listaAlunos={listaAlunosMemo} />
                                            </div>

                                            <div className="matriculas-matriculados" id="importar-alunos-button">
                                                <h3>Alunos Matriculados: ({fields.length})</h3>
                                                <button
                                                    type="button"
                                                ...
                                                <AlunosMatriculados
                                                    alunosForaDaFaixa={alunosForaDaIdadeMemo}
                                                    fieldIndex={fieldIndex}
                                                    listaAlunosSelecionados={listaAlunosSelecionadosMemo}
                                                    register={register}
                                                    remove={remove}
                                                />
                                            </div>
                                        ...
                                        <div
                                            className={`novo-trimestre__actions ${
                                                licaoReference && !isSecretario.current
                                        ...
                                        >
                                            ...
                                            <div className="novo-trimestre__actions-btn">
                                                ...
                                                <button type="submit" className="button-primary">
                                                    {licaoReference ? "Salvar" : "Criar Trimestre"}
                                                </button>
                                            </div>
                                        </div>
                                    </form>
                    ...
                    <CadastroAlunoModal
                        ...
                        onSave={(v) => {
                            setAlunosMap(
                                (a) =>
                                    new Map([
                                        ...Array.from(a.values()).map((v) => [v.id, v]),
                                        [
                                            v.id,
                                            {
                                                nome: v.nome_completo,
